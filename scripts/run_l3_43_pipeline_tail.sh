...
log_stage() {
  echo "$(date --iso-8601=seconds) STAGE tail run=$(basename "${RUN_DIR}") ${*}" >&2
}
...
EVAL_BATCH_SIZE="${STGPT_EVAL_BATCH_SIZE:-32}"
export STGPT_EVAL_SIM_CHUNK_SIZE="${STGPT_EVAL_SIM_CHUNK_SIZE:-256}"
export STGPT_EVAL_SILHOUETTE_MAX_REGIONS="${STGPT_EVAL_SILHOUETTE_MAX_REGIONS:-4096}"

trap 'status=$?; echo "$(date --iso-8601=seconds) ERROR tail run=$(basename "${RUN_DIR}") line=${LINENO} status=${status}" >&2' ERR

export CUDA_VISIBLE_DEVICES="${GPU}"

if [[ ! -s "${RUN_DIR}/validate_data.json" || ! -s "${SPLITS}" ]]; then
  echo "Run is missing validated QC artifacts: ${RUN_DIR}" >&2
  exit 3
fi
...
log_stage "step=check-contract checkpoint=${CKPT} gpu=${GPU}"
...
log_stage "step=evaluate checkpoint=${CKPT} gpu=${GPU} batch_size=${EVAL_BATCH_SIZE} sim_chunk_size=${STGPT_EVAL_SIM_CHUNK_SIZE} silhouette_max_regions=${STGPT_EVAL_SILHOUETTE_MAX_REGIONS}"
run_stgpt evaluate --checkpoint "${CKPT}" --config "${CONFIG_PATH}" --splits "${SPLITS}" --output "${RUN_DIR}/evaluation" --batch-size "${EVAL_BATCH_SIZE}" --device cuda | tee "${RUN_DIR}/evaluation_stdout.json"

log_stage "step=export-spatho checkpoint=${CKPT} gpu=${GPU} batch_size=${EVAL_BATCH_SIZE}"
run_stgpt export-spatho --checkpoint "${CKPT}" --config "${CONFIG_PATH}" --output "${RUN_DIR}/spatho_export" --batch-size "${EVAL_BATCH_SIZE}" --device cuda | tee "${RUN_DIR}/spatho_export_stdout.json"
